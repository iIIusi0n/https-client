...
namespace Http {
SslClient::SslClient(const wchar_t *hostname, int port) {
...
void SslClient::SetMethod(const wchar_t *method) {
  method_ = new wchar_t[wcslen(method) + 1];
  wcscpy(method_, method);
}

void SslClient::SetPath(const wchar_t *path) {
  path_ = new wchar_t[wcslen(path) + 1];
  wcscpy(path_, path);
}

void SslClient::SetUserAgent(const wchar_t *user_agent) {
  user_agent_ = new wchar_t[wcslen(user_agent) + 1];
  wcscpy(user_agent_, user_agent);
}

void SslClient::SetReferer(const wchar_t *referer) {
  referer_ = new wchar_t[wcslen(referer) + 1];
  wcscpy(referer_, referer);
}

void SslClient::SetAccept(LPCWSTR *accept) {
  accept_ = accept;
}

void SslClient::AddRequestHeader(const wchar_t *header) {
...
void SslClient::AddCustomHeader(const wchar_t *name, const wchar_t *value) {
...
void SslClient::AddBodyData(const wchar_t *content_type, const char *content,
